...
private:
	...
	//Priority_queue<eventClass> myPQ;
...
public:
	...
	BOOLEAN existsAvailablePump () {return myPumps.size() > 0;};
...
{
	//cout << "entered pumpstandclass" << endl;
	if (N < 1) {
	...
	for (int j = 0; j < N; j++)
		...
		myPumps.push(new pumpClass);
		//cout << "pump size: " << myPumps.size() << endl;
}
...
void carQueueClass::insert (carClass * newestCar)
{
	...
	myQueue.push(newestCar);
	if (myQueue.size() > queueMax)
	{
		queueMax = myQueue.size();
	}
	/*
...
carClass * carQueueClass::getNext ()
{
	...
	carClass * carToReturn = myQueue.front(); 
	myQueue.pop();
...
int main()
{
	...
	cout << "set ReportInterval and EndingTime" << endl;
	cin >> ReportInterval >> endingTime;

	int numPumps;
	cout << "specify Number of pumps" << endl;
	cin >> numPumps;
	...
	cout << "Max Queue size: " << queueMax << endl;
